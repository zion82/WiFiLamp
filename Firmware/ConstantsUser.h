...
// === РЕЖИМЫ РАБОТЫ  ===================
#define AUTOMATIC_OFF_TIME    (3 * 60UL * 60UL * 1000UL)    // раскомментируйте эту строку, если нужно, чтобы лампа выключалась сама после 5 часов непрерывной работы (вдруг кто забыл перед отъездом выключить её). в приложении время будет отображаться некорректно!
...
#define EFF_FAV = EFF_NEXUS                                 // любимый эффект под него будет отдельная кнопка в десктопном приложении по умолчанию будет выводиться версия прошивки
...
// #define RUNNING_TEXT_DEFAULT  ("2.2 beta 86")               // текст, который будет выводиться в эффекте Бегущая строка по умолчанию. его можно менять в приложении по 
